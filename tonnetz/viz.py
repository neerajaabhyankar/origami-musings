...
import numpy as np
from vedo import Plotter, Point, Points, Text3D, Axes, Line
...
# )
points = [
    Point(list([tn.coords3d[aa][ii] for aa in range(len(tn.coords3d))]), r=SPHERE_SIZE, c="navy")
    for ii in range(len(tn.node_names))
]
ratio_to_swarval = lambda ff: (ff - 1) * 12
points_elevations = [ratio_to_swarval(tn.node_frequencies[ii]) for ii in range(len(tn.node_names))]
plotter += points

# Text
texts = [
    Text3D(name, pos=(tn.coords3d[0][i], tn.coords3d[1][i], tn.coords3d[2][i]+0.5), s=0.2, c="white", justify="center")
...
for y in range(RANGE["Y"][0], RANGE["Y"][1]):
    for z in range(RANGE["Z"][0], RANGE["Z"][1]):
        line = Line((RANGE["X"][0], y, z), (RANGE["X"][1], y, z), c="lightgray", alpha=0.2)
        plotter += line
for x in range(RANGE["X"][0], RANGE["X"][1]):
    for z in range(RANGE["Z"][0], RANGE["Z"][1]):
        line = Line((x, RANGE["Y"][0], z), (x, RANGE["Y"][1], z), c="lightgray", alpha=0.2)
        plotter += line
for x in range(RANGE["X"][0], RANGE["X"][1]):
    for y in range(RANGE["Y"][0], RANGE["Y"][1]):
        line = Line((x, y, RANGE["Z"][0]), (x, y, RANGE["Z"][1]), c="lightgray", alpha=0.2)
        plotter += line
...
for angle in range(90):  # 90 steps to rotate by 90 degrees
    ...
    for pp, point in enumerate(points):
        ...
        texts[pp].pos([pos[0]+0.5*np.sin(np.radians(angle)), pos[1], pos[2] + points_elevations[pp]/90 + 0.5*np.cos(np.radians(angle))])
